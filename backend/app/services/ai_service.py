...
Чеклист: даты + кол-во гостей + ФИО + телефон. Без любого — спроси!""")
        ...
        # PMS-specific note (#21) — what happens AFTER the manager confirms
        pms_kind = (hotel_data.get('pms_kind') or 'none').lower()
        pms_messages = {
            'exely': (
                "\n\n## PMS\nОтель ведёт бронирования в Exely. После подтверждения менеджером "
                "бронь автоматически появится в Exely — гостю не нужно отдельно регистрироваться."
            ),
            'altegio': (
                "\n\n## PMS\nОтель ведёт бронирования в Altegio. После подтверждения менеджером "
                "бронь будет занесена в Altegio."
            ),
            'shelter': (
                "\n\n## PMS\nОтель ведёт бронирования в Shelter. После подтверждения менеджером "
                "бронь будет занесена в Shelter."
            ),
            'custom': (
                "\n\n## PMS\nОтель использует собственную систему учёта. После подтверждения "
                "менеджером бронь регистрируется в их внутренней CRM."
            ),
            'none': (
                "\n\n## PMS\nОтель пока без PMS — менеджер ведёт бронирования вручную "
                "(Google Sheets / блокнот). Не обещай гостю «забронировано в системе» — "
                "только «передаю менеджеру для подтверждения»."
            ),
        }
        parts.append(pms_messages.get(pms_kind, pms_messages['none']))
