...
## ADDENDUM 2026-04-25 (раннее утро) — sales + training prep

После закрытия Sprint 1 ceremonies сессия продолжилась — Алан запросил ещё 3 трека:

### 1. Бизнес-тренинг pitch (`docs/sales/business_training_pitch_2026_04.md`)

Создан один-страничный документ с 30-сек/2-мин elevator pitch + 7 типичных вопросов тренера + 6 каверзных вопросов + 4 истории + финансовые цифры (CAC $300, LTV $1760, gross margin 85%) + reframing 3 слабостей (1 клиент / solo founder / маленький рынок) + анти-паттерны.

**Использование:** Алан берёт на бизнес-тренинг как mental cheat-sheet. Можно показывать в любом sales-разговоре или интервью.

### 2. Sales presentation review (Perplexity ответ)

Алан показал ответ Perplexity на анализ HTML-презентации (17 слайдов) для Назиры. **Команда (Caravaggio + Mary + Victor + Sally + John + Sophia) дала вердикт:**

- 🔴 **CRITICAL:** Perplexity использовал устаревшую цену $500+$15. Текущая $800+$40. На старой цене Назира продаёт в минус (margin отрицательная на отелях >40 ном).
- 🔴 **CRITICAL:** Perplexity нагалюцинировал цифры ("377 отелей в Чолпон-Ате", "2.3M туристов", "ROI 300%", "148k турпредприятий КР", "+30% лидов") — все опасны для встречи с владельцем который знает рынок.
- 🟡 **Слабая позиция:** "мы дешевле" вместо нашей сильной "monopolist by design в нише mini-hotel КР".
- ✅ Хорошие идеи: ROI-формула, FAQ-слайд, "до/после" визуал, обрезать до 10-12 слайдов.

**Решение:** не использовать ответ Perplexity as-is. Сделать вторую итерацию с правильным контекстом.

### 3. Готовый промпт для Perplexity v2 (`docs/sales/perplexity_prompt_2026_04.md`)

Создан полный промпт для копи-паста в Perplexity, содержащий:
- Актуальную цену $800+$40 + Назире 37.5% = $300
- Реальные метрики Ton Azure (58 диалогов/мес, 82% автоматизация, 94% RU / 6% KG)
- Полную intel-картину конкурентов (HotelBot $2670/$6675, NURAI universal CRM, и др.)
- 8 задокументированных багов HotelBot как sales-арсенал
- ICP / TAM / контекст продажи Назиры
- Явный запрет на $500/$15, выдуманные цифры, generic SaaS-метрики

**Использование:** Алан копирует в Perplexity, получает ответ, кидает в чат — Claude разбирает с командой ещё раз.

### 4. Trello — без изменений за этот блок

Не было движений по доске в этом ADDENDUM. Доска осталась в состоянии: Backlog 39 / In Progress 1 / Done 21.

---

## Bonus вопросы которые Алан задал и стоит запомнить

1. **«Когда добьём Ex-Machina?»** — Ответил: «не добьём, эволюционирует» (insight #1). Реалистичный таймлайн: R1a code-complete конец мая 2026, 1-й новый клиент июнь, 3 платящих без churn октябрь, R2 (Exely) декабрь, scaled SaaS 2027.

2. **«Смогу ли поехать в Силиконовую долину показать Ex-Machina?»** — Ответил: SV не правильный target для hotel-vertical (там consumer/finance/dev-tools). Лучше HITEC Хьюстон / ITB Berlin / Arabian Travel Market Дубай. SV через YC W27 (Oct 2026 application) если будет $5-10k MRR — реалистично 2027.

3. **«Что в итоге сделали сегодня?»** — Ответ задокументирован выше (5 коммитов, 7 Trello-движений, memory cleanup).

---

## Финальный финал — что делать в понедельник

Действия не изменились, повторю для ясности:

1. Sprint 2 planning через party-mode (готовые AC у `#26 / #25 / #17`)
2. Если ответ программистов HotelBot или NURAI пришёл за выходные — обработать первым
3. Перед стартом `#26` (L-story) — party-mode по новому DoR
4. Если Алан запустил Perplexity v2 за выходные — кинет ответ → разберём с командой

Также может Алан захочет **обновить HTML-презентацию** на основе Perplexity v2 ответа + наших sales-asset'ов. Это отдельный трек на Sprint 2 (низкий приоритет если #26 идёт активно).
